...
public interface EmailHistoryRepository extends CrudRepository<EmailHistoryEntity,Integer> {
    @Query("select new com.example.mappers.EmailHistoryMapper(c.id, c.to_email ,c.title , c.message ,c.createdDate ) " +
            "from EmailHistoryEntity c ")
    Page<EmailHistoryMapper> getList(Pageable pageable);

    @Query("select new com.example.mappers.EmailHistoryMapper(c.to_email ,c.title , c.message ,c.createdDate ) " +
            "from EmailHistoryEntity c ")
